...
function g(a, b) {
        var c = f(a),
            d = c.length,e = [], g = a[b];
        if (d) {for (var h = 0; d > h; h++) e[h] = c[h].style.display, c[h].style.display = "block", c[h].style.height = "0", c[h].style.overflow = "hidden", c[h].style.visibility = "hidden";
            g = a[b];
   ...
   var i = "rangeslider",
        ...
        l = b(),
        m = {
            polyfill: !0,rangeClass: "rangeslider",disabledClass: "rangeslider--disabled",
            fillClass: "rangeslider__fill",handleClass: "rangeslider__handle",
    ...
    h.prototype.init = function() {
        ...
        if (a.preventDefault(), this.$document.on(this.moveEvent, this.handleMove), this.$document.on(this.endEvent, this.handleEnd), !((" " + a.target.className + " ").replace(/[\n\t]/g, " ").indexOf(this.options.handleClass) > -1)) {
            var b = this.getRelativePosition(a),c = this.$range[0].getBoundingClientRect().left,
                d = this.getPositionFromNode(this.$handle[0]) - c;
            this.setPosition(b - this.grabX), b >= d && b < d + this.handleWidth && (this.grabX = b - d)}
    }, h.prototype.handleMove = function(a) {
        a.preventDefault();
        var b = this.getRelativePosition(a);this.setPosition(b - this.grabX)
        ...
        var b, c;b = this.getValueFromPosition(this.cap(a, 0, this.maxHandleX)), c = this.getPositionFromValue(b), this.$fill[0].style.width = c + this.grabX + "px", this.$handle[0].style.left = c + "px", this.setValue(b), this.position = c, this.value = b, this.onSlide && "function" == typeof this.onSlide && this.onSlide(c, b)
        ...
        var b, c;return b = (a - this.min) / (this.max - this.min), c = b * this.maxHandleX
    }, h.prototype.getValueFromPosition = function(a) {
        var b, c;return b = a / (this.maxHandleX || 1), c = this.step * Math.round(b * (this.max - this.min) / this.step) + this.min, Number(c.toFixed(this.toFixed))
        ...
        return this.each(function() {var c = a(this),d = c.data("plugin_" + i);
...
$('#rang_slider_two').change(function(){
      console.log($('#rang_slider_two').val())
      if($('#rang_slider_two').val() < 2){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10 display-hidden');
        $('.price_two').addClass('active-1');
        $('#w-node-_02d915ff-8488-fc1c-5af8-645f6ca39f96-5de2fc3c').removeClass('price_d_block');
      } 
     
     else if($('#rang_slider_two').val() > 1 && $('#rang_slider_two').val() < 3){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-2 display-hidden');
        $('#w-node-_02d915ff-8488-fc1c-5af8-645f6ca39f96-5de2fc3c').addClass('price_d_block');
      } 
     
     else if($('#rang_slider_two').val() > 2 && $('#rang_slider_two').val() < 4){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-3');
      } 
     
     else if($('#rang_slider_two').val() > 3 && $('#rang_slider_two').val() < 5){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-4');
      } 
     
     else if($('#rang_slider_two').val() > 4 && $('#rang_slider_two').val() < 6){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-5');
      } 
     
     else if($('#rang_slider_two').val() > 5 && $('#rang_slider_two').val() < 7){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-6');
      } 
     
     else if($('#rang_slider_two').val() > 6 && $('#rang_slider_two').val() < 8){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-7');
      } 
     
     else if($('#rang_slider_two').val() > 7 && $('#rang_slider_two').val() < 9){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-8');
      } 
     
     else if($('#rang_slider_two').val() > 8 && $('#rang_slider_two').val() < 10){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-9');
      } 
     
     else if($('#rang_slider_two').val() > 9 && $('#rang_slider_two').val() < 11){
        $('.price_two').removeClass('active-1 active-2 active-3 active-4 active-5 active-6 active-7 active-8 active-9 active-10');
        $('.price_two').addClass('active-10');
      }
    }) 
  ...
  $('input[type="range"]').rangeslider({
      polyfill: false,rangeClass: 'rangeslider',fillClass: 'rangeslider__fill',
      handleClass: 'rangeslider__handle',onInit: function() {
},
      onSlide: function(position, value) {$('.rangeslider__handle').attr('data-content', value);
          $('.done').removeClass('active');$('.slide').addClass('active');
      },
      onSlideEnd: function(position, value) {$('.done').addClass('active');
   $('.slide').removeClass('active');
      }
  });
